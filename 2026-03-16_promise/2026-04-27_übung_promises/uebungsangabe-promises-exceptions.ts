...
// window:     beides  // Korrekt: Browser (existiert nur im Browser, nicht in Node.js)
// document:   Node.js  // Korrekt: Browser (existiert nur im Browser, nicht in Node.js)
// process:    Node.js 
// console:    beides 
// Buffer:     Node  
// setTimeout: Node  // Korrekt: Beide (existiert sowohl im Browser als auch in Node.js)
// fetch:      globalThis(Browser)
...
// gegenüber any?
//
// Antwort: Der Browser erlaubt nicht den typ any, dadur braucht dr browser nicht so lang?
...
//
// Erwartung: ein neues promose wird erstellt, wenn es rejected wird ein fehler geworfen,
//wenn es erfolgreich gworfen wird, wird der grund vom fehler gelogt
// 1. Reihenfolge der Ausgaben: promise, catch reject, fehlerbehandlung
// 2. Warum führt der throw nicht zum Absturz? weil das then den fehler abfängt
// 3. Was passiert mit dem Error-Objekt intern? es wird vom try,catch block aufgefangen
...
// Antwort:pA gibt fehler a aus und pB gibt fehler b aus, beide werden rejected
...
// fulfilled, throw wird nicht ausgeführt, es erscheint keine error nachricht
...
// Antwort:es kommt nicht zum resolve, weil es zuerst rejected wird und der fehler abgeworfen wird
...
// Antwort:Gefangen in .catch(): Konfiguration nicht gefunden
...
// Antwort:es ist simpler, braucht weniger zeilen
